...
app.post("/api/login", (req, res) => {
  ...
  if (
    index >= 0 &&
    !Array.isArray(list[index].contacts)
  ) {
    list[index].contacts = [];
    saveUsers(list);
  }
...
app.post(
  ...
  (req, res) => {
    ...
    list[index].contacts.push(target);
...
app.post(
  ...
  (req, res) => {
    ...
    if (!alreadyViewed) {
      ...
      // Informar al dueño de que alguien ha visto
      const ownerSocket =
        getSocketId(
          story.username
        );
